...
def main():
    ...
    try:
        thread_count = int(input(f"\n{Colors.CYAN}[?] Number of threads (recommended: 10-5000): {Colors.END}") or "50")
        # Validation for thread count between 1-5000
        if thread_count < 1:
            thread_count = 1
        elif thread_count > 5000:
            thread_count = 5000
            print(f"{Colors.YELLOW}[!] Thread count limited to 5000.{Colors.END}")
